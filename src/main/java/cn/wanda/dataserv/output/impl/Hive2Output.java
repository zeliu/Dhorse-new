...
import cn.wanda.dataserv.config.location.Hive2OutputLocation;
...
@Log4j
public class Hive2Output extends AbstractOutput {
...
//		}
    ...
    private Hive2OutputLocation location;
    ...
    public void init() {
        ...
        if (!Hive2OutputLocation.class.isAssignableFrom(l.getClass())) {
            throw new OutputException("output config type is not Hive2!");
        }

        ClassLoader cl = ObjectFactory.dpumpClassLoader.get(Hive2Output.class
                .getName());
        if (cl != null) {
            log.info("set classloader in threadlocal.");
            Thread.currentThread().setContextClassLoader(cl);
        }

        DFSUtils.loadHadoopNative(DFSUtils.HHDFS);

        location = (Hive2OutputLocation) l;
...
//		fieldDelim = this.outputConfig.getSchema().getFieldDelim();
        ...
        // get hive conf
        try {
            //conf = HiveUtils.getConf(ugi, hiveSite, hadoopSite, HiveUtils.BHIVE2);
            conf = HiveUtils.getConf(location.getHdfs(), location.getHive(), HiveUtils.HHIVE);
        } catch (IOException e) {
            throw new OutputException(String.format(
                    "Initialize hive2 conf failed:%s,%s", e.getMessage(),
                    e.getCause()), e);
        }
        ...
        Class<? extends HiveOutputFormat> ofClazz = t.getOutputFormatClass();
        ...
        try {
            ...
            HiveOutputFormat hof = ofClazz.newInstance();
            this.adapter.open(hof);
        } catch (Exception e) {
            throw new OutputException(String.format(
                    "Initialize hive2 adapter failed:%s,%s", e.getMessage(),
                    e.getCause()), e);
        }
    ...
    public void post(boolean success) {
        ...
        log.info("do post");
        try {
            location = (Hive2OutputLocation) this.outputConfig.getLocation();
            ...
            throw new OutputException("Hive2Output write hive metadata failed.", e);
        } catch (IOException e) {
            throw new OutputException("Hive2Output get filesystem failed.", e);
        } catch (Exception e) {
            throw new OutputException("Hive2Output dopost failed.", e);
        }
    }
    ...
    abstract class AbstractHiveOutputFormatAdapter implements
        ...
        public void open(HiveOutputFormat hof) {
            try {
                ...
                throw new OutputException("Hive2Output get RecordWrite failed.",
                        e);
            }
        }
        ...
        public void close() {
            try {
                ...
                log.warn("Hive2Output close failed.", e);
            }
    ...
    class RCFileOutputFormatAdapter extends AbstractHiveOutputFormatAdapter
        ...
        public void write(Line line) {
            try {
                ...
                throw new OutputException("Hive2Output write failed.", e);
            }
        }
