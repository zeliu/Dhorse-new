package cn.wanda.dataserv.utils.elasticsearch.node;

import cn.wanda.dataserv.utils.elasticsearch.*;
...
public class BulkNodeClient implements Ingest {
    ...
    public BulkNodeClient newClient(Client client) {
        ...
        BulkProcessor.Listener listener = new BulkProcessor.Listener() {
            ...
            public void beforeBulk(long executionId, BulkRequest request) {
                ...
                log.debug(String.format("before bulk [%d] [actions=%d] [bytes=%d] [concurrent requests=%d]",
                        executionId,
            ...
            public void afterBulk(long executionId, BulkRequest request, BulkResponse response) {
                ...
                log.debug(String.format("after bulk [%d] [succeeded=%d] [failed=%d] [%d ms] %d concurrent requests",
                        executionId,
                        ...
                        l));
                if (n > 0) {
                    log.error(String.format("bulk [%d] failed with %d failed items, failure message = %s",
            ...
            public void afterBulk(long executionId, BulkRequest request, Throwable failure) {
                ...
                throwable = failure;
                closed = true;
                log.error("after bulk [" + executionId + "] error", failure);
            }
        };
    ...
    public BulkNodeClient putMapping(String index) {
        if (client == null) {
            log.warn("no client for put mapping");
            return this;
        }
    ...
    public BulkNodeClient deleteMapping(String index, String type) {
        if (client == null) {
            log.warn("no client for delete mapping");
            return this;
        }
    ...
    public BulkNodeClient index(String index, String type, String id, String source) {
        ...
        try {
            ...
            throwable = e;
            closed = true;
            log.error("bulk add of index request failed: " + e.getMessage(), e);
        } finally {
            if (metric != null) {
                metric.getCurrentIngest().dec();
            }
    ...
    public BulkNodeClient delete(String index, String type, String id) {
        ...
        try {
            ...
            throwable = e;
            closed = true;
            log.error("bulk add of delete failed: " + e.getMessage(), e);
        } finally {
            if (metric != null) {
                metric.getCurrentIngest().dec();
            }
    ...
    public synchronized void shutdown() {
        try {
            if (bulkProcessor != null) {
                log.debug("closing bulk processor...");
                bulkProcessor.close();
            }
            if (metric != null && metric.indices() != null && !metric.indices().isEmpty()) {
                log.debug(String.format("stopping bulk mode for indices %d...", metric.indices()));
    ...
    public BulkNodeClient newIndex(String index, Settings settings, Map<String, String> mappings) throws IOException {
        ...
        if (client == null) {
            log.warn("no client for create index");
            return this;
        }
        if (index == null) {
            log.warn("no index name given to create index");
            return this;
        }
        ...
        if (concreteSettings != null) {
            log.info(String.format("newIndex: settings = %s", concreteSettings.getAsMap()));
        ...
        if (mappings == null && getMappings() != null) {
            for (String type : getMappings().keySet()) {
                log.info(String.format("newIndex: type = %s mappings = %s", type, getMappings().get(type)));
                createIndexRequestBuilder.addMapping(type, getMappings().get(type));
            }
        } else if (mappings != null) {
            for (String type : mappings.keySet()) {
                log.info(String.format("newIndex: type = %s mappings = %s", type, mappings.get(type)));
    ...
    public BulkNodeClient deleteIndex(String index) {
        ...
        if (index == null) {
            log.warn("no index name given to delete index");
            return this;
        }
