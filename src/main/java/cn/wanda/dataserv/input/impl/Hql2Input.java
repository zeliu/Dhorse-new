...
import cn.wanda.dataserv.config.location.Hql2InputLocation;
...
@Log4j
public class Hql2Input extends AbstractInput {
    ...
    public void init() {
        ...
        if (!Hql2InputLocation.class.isAssignableFrom(l.getClass())) {
            throw new InputException("input config type is not HQL2!");
        }

        ClassLoader cl = ObjectFactory.dpumpClassLoader.get(Hql2Input.class
                .getName());
        if (cl != null) {
        ...
        Hql2InputLocation location = (Hql2InputLocation) l;

        try {
            this.conf = HiveUtils.getConf(location.getHdfs(), location.getHive(), HiveUtils.HHIVE);
                ...
                public void run() {
                    try {
                        ...
                        try {
                            ...
                            InputWorker.INPUT_ERROR.put(inputConfig.getId(), ex);
                        }
                    }
                }
            };
    ...
    private void processHql(String cmd, CommandProcessor proc, SessionState ss) throws Exception {
        if (proc != null) {
    ...
    public void close() {
        ...
        try {
            ...
            throw new InputException("close hql2 input failed.", ex);
        }
    }
